...
async def download(session, url, start, end):
...
async def download_data(date, cycle, step, req_param, req_level, output_file, mode='wb'):
    ...
    idx_df = await parse_idx(idx_link)
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [download(session, pgrb2_link, record["start"], record["end"]) for record in idx]
        tasks_with_progress = atqdm.as_completed(tasks, total=len(tasks))
        with open(output_file, mode) as f:
            for task in tasks_with_progress:
                content = await task
                f.write(content)

async def parse_gefs_idx(idx_link):
    async with aiohttp.ClientSession() as session:
        r = await fetch(session, idx_link)
    data = StringIO(r)
...
async def download_gefs(date, cycle, step, req_param, req_level, output_file, mode='wb'):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for member in members:
            ...
            tasks.append(parse_gefs_idx(idx_link))
    ...
    if req_level is None:
        ...
        idx_df = idx_df[(idx_df.param.isin(req_param)) & (idx_df.level.isin(req_level))]
    idx = idx_df.to_dict(orient="records")
    pgrb2_link = idx_link.replace('.idx','')
    
    async with aiohttp.ClientSession() as session:
        tasks = [download(session, record['link'], record["start"], record["end"]) for record in idx]
        tasks_with_progress = atqdm.as_completed(tasks, total=len(tasks))
        with open(output_file, mode) as f:
            for task in tasks_with_progress:
                content = await task
...
async def ingest(date, cycle, step, gefs):
    req_param = ["gh", "t"]
